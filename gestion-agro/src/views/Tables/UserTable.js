...
export default function UserTable() {
  ...
  const handleAddRow = async (post) => {
    ...
    await ApiPostRequest(`http://${dataBaseIp}:${dataBasePort}/api/usuario/create`, post)
    fetchData();
    setModal(false);
  };
  ...
  const handleUpdateRow = async (post, id) => {
    ...
    await ApiUpdateRequest(`http://${dataBaseIp}:${dataBasePort}/api/usuario/update/${id}`, post)
    fetchData();
    setModal(false);
  };
  ...
  const fetchData = async () => {
    await ApiGetRequest(`http://${dataBaseIp}:${dataBasePort}/api/usuario/list`)
      .then(data => {
  ...
  const handleModal = (selected = [], modalData = "") => {
    setData(selected);
  ...
  const contextActions = useMemo(() => {
    ...
    const handleDelete = () => {
      ...
      if (window.confirm(`Seguro que deseas eliminar datos del id ${selectedRows.map(r => r.id)}?`)) {
        ...
        selectedRows.map((row) => {
          ApiDeleteRequest(`http://${dataBaseIp}:${dataBasePort}/api/usuario/delete/${row.id}`);
        });
    ...
    //actualizar fila
    const handleUpdate = () => {
      handleModal(selectedRows, "update");
      setToggleCleared(!toggleCleared);
    };
      ...
      {!resultInfo?.data ?
        ...
        <Box>
          ...
          >
            ...
            <UsuarioModal setModal={setModal}
              rowData={!modalData ? [] : data[0]} //se envian los datos ya existente si se esta actualizando
              modalData={modalData} //tipo de peticion que se va a realizar -> create o update
