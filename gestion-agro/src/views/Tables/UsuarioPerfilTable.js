...
export default function UsuarioPerfilTable() {
  ...
  const handleAddRow = async (post) => {
    ...
    await ApiPostRequest(`http://${dataBaseIp}:${dataBasePort}/api/usuarioperfil/create`, post)
    console.log(post)
    fetchData();
    setModal(false);
  };
  ...
  const handleUpdateRow = async (post, id) => {
    ...
    await ApiUpdateRequest(`http://${dataBaseIp}:${dataBasePort}/api/usuarioperfil/update/${id}`, post)
    fetchData();
    setModal(false);
  };
  ...
  const fetchData = async () => {
    await ApiGetRequest(`http://${dataBaseIp}:${dataBasePort}/api/usuarioperfil/list`)
      .then(data => {
    ...
    const handleDelete = () => {
      ...
      if (window.confirm(`Seguro que deseas eliminar datos del id ${selectedRows.map(r => r.id)}?`)) {
        ...
        selectedRows.map((row) => {
          ApiDeleteRequest(`http://${dataBaseIp}:${dataBasePort}/api/usuarioperfil/delete/${row.id}`);
        });
      ...
      {!resultInfo?.data ?
        ...
        <Box>
          ...
          >
            ...
            <UsuarioPerfilModal setModal={setModal}
